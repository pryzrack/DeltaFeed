...
function main(data) {
    for (let feed of data.feeds){
        $('feeds').innerHTML += `<div class="card">
        <header>
    ...
    for (let lang of data.languages){
        $('langs').innerHTML += `<option value="${lang[1]}">${lang[0]}</option>`;
    };
    for (let console of data.consoles){
        $('consoles').innerHTML += `<option value="${console}">${console}</option>`;
    };
    for (let i of BOTS.nv){
        $('nv').innerHTML += `<option value="${i}">${i}</option>`;
    };
    for (let i of BOTS.fd){
        $('fd').innerHTML += `<option value="${i}">${i}</option>`;
    }
    for (let i of BOTS.tr){
        $('tr').innerHTML += `<option value="${i}">${i}</option>`;
    }
}
onload = () => {
